...
class SessionNotifier:
    def __init__(self):
        ...
        self.schedule = {
            (16, 0): (None, "Début de session"),
            (16, 37): ("son1.wav", "Il reste 30 minutes"),
            (16, 38): ("son1.wav", "Il reste 15 minutes"),
            (16, 39): ("son1.wav", "Il reste 10 minutes"),
        ...
        with self.tts_lock:
            try:
                ...
                if "run loop already started" in str(e):
                    # Recréer le moteur TTS en cas d'erreur
                    try:
                        self.engine.stop()
                        self.engine = pyttsx3.init()
                        ...
                        self.engine.say(text)
                        self.engine.runAndWait()
                        print(f"🗣️ Message prononcé (après récupération) : {text}")
                    except Exception as e2:
                        print(f"❌ Erreur récupération synthèse vocale : {e2}")
                else:
