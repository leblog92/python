...
"""
═══════════════════════════════════════════════════════════════════════════
  NICHOIR — Surveillance 24/24 d'un couple de pigeons ramiers
  Corniche de la médiathèque
═══════════════════════════════════════════════════════════════════════════
 
Adapté à partir du script "flux_jvo.py" (contrôle de la salle jeux vidéo).
 
CE QUI A ÉTÉ RETIRÉ (spécifique à la salle JVO, sans rapport avec un nichoir) :
  - Annonces vocales / TTS (Edge TTS + pyttsx3) et bibliothèque de phrases
  - Bibliothèque MP3 (upload, tags, lecture, minuteur d'horaires de salle)
  - Reconnaissance faciale des visiteurs
  - Interphone (micro du visiteur → haut-parleurs de la salle)
  - "Mode absent" (lié aux annonces sonores, devenu sans objet)
 
CE QUI A ÉTÉ GARDÉ (adapté) :
...
CE QUI A ÉTÉ AJOUTÉ (pour une surveillance 24/24 sans surveillant humain) :
...
CONFIGURATION (fichier .env à côté de ce script, toutes les clés sont
optionnelles) :
  NICHOIR_PASSWORD          mot de passe de l'interface web (vide = accès libre, LAN uniquement)
  ...
  NICHOIR_NETWORK_HTML_DIR   dossier réseau où déposer nichoir.html (repli en local sinon)
...
# ─────────────────────────────────────────────
#  SYSTÈME DE LOGS
#  Écrit dans la console ET dans nichoir_debug.log
#  (dans le même dossier que ce script)
# ─────────────────────────────────────────────
LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nichoir_debug.log")
...
APP_PASSWORD = os.environ.get('NICHOIR_PASSWORD', '')
...
def save_html_file():
    """
    Génère un fichier nichoir.html qui redirige simplement vers http://IP:5000.
    Ainsi, depuis n'importe quel poste du LAN, ouvrir nichoir.html ouvre
    l'interface complète servie par Flask — sans problème de sécurité file://.
    Le dossier réseau de destination se configure via NICHOIR_NETWORK_HTML_DIR
    dans le .env ; à défaut (ou si inaccessible), le fichier est écrit en local.
    """
...
<head>
  ...
  <title>Redirection Nichoir…</title>
</head>
<body>
  ...
  <script>window.location.replace("{server_url}");</script>
    ...
    destination_path = os.environ.get('NICHOIR_NETWORK_HTML_DIR', '').strip()
    if not destination_path or not os.path.exists(destination_path):
        if destination_path:
            print("Chemin réseau inaccessible, sauvegarde locale.")
        destination_path = "."
 
    html_file_path = os.path.join(destination_path, "nichoir.html")
    try:
        with open(html_file_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        print(f"Fichier nichoir.html généré : {html_file_path}")
    except Exception as e:
        print(f"Impossible d'écrire nichoir.html : {e}")
...
capture_count    = 0
MAX_CAPTURES     = 300        # surveillance 24/24 → plus de place que pour une salle
SAVE_CAPTURES    = True       # activé par défaut : on veut les événements du nid dès le démarrage
 
user_profile = os.environ.get('USERPROFILE', os.path.expanduser('~'))
motion_captures_dir = os.path.join(user_profile, 'Pictures', 'nichoir_captures')
...
#  éclosion, envol...).
# ─────────────────────────────────────────────
TIMELAPSE_DIR = os.path.join(user_profile, 'Pictures', 'nichoir_timelapse')
...
SNAPSHOTS_DIR = os.path.join(user_profile, 'Pictures', 'nichoir_snapshots')
...
<title>Nichoir Pigeon Ramier – Médiathèque</title>
<style>
:root {
...
<header>
  <div class="dot"></div>
  <h1>🐦 NICHOIR — Pigeon ramier &ndash; M&eacute;diath&egrave;que</h1>
</header>
...
if __name__ == '__main__':
    log.info("=== NICHOIR PIGEON RAMIER — SURVEILLANCE 24/24 ===")
